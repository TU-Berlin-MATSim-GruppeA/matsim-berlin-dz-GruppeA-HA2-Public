package org.matsim.run.GroupA.PlanA.PlanA_Version1;
...
/**
speed und lane changed
 */
public class NetworkModifier_PlanA_Version1 {
    private static final Logger LOG = Logger.getLogger(NetworkModifier_PlanA_Version1.class);
    ...
    public static void main (String[] args) throws IOException {
        ...
        for (int i = 0; i < RedStreetsList.size(); i++) {
            if(cleanedNetworkLinkIDList.contains(RedStreetsList.get(i))) {
                double numberOfLanes_After_RedStreets = scenario.getNetwork().getLinks().get(Id.createLinkId(RedStreetsList.get(i))).getNumberOfLanes() + 1.;
                scenario.getNetwork().getLinks().get(Id.createLinkId(RedStreetsList.get(i))).setNumberOfLanes(numberOfLanes_After_RedStreets);

                double capacity_After_RedStreets = scenario.getNetwork().getLinks().get(Id.createLinkId(RedStreetsList.get(i))).getCapacity() + 1800.;
                scenario.getNetwork().getLinks().get(Id.createLinkId(RedStreetsList.get(i))).setCapacity(capacity_After_RedStreets);
            }
        }
