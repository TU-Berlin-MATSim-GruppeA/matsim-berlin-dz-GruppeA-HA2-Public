...
public final class RunBerlinScenarioWithSubPop_GroupA {

	private static final Logger log = Logger.getLogger(RunBerlinScenarioWithSubPop_GroupA.class );
	...
	public static void main(String[] args) {
		...
		if ( args.length==0 ) {
			...
			//更改config里的plan file input from URL
			args = new String[] {"scenarios/berlin-v5.5-1pct/input/berlin-v5.5-1pct.config-subpop_GroupA.xml"}  ;
	...
	public static Scenario prepareScenario( Config config ) {
		...
		//在Runner里 change plan file
		//for AgentLists
		List<String> agentIDList = new ArrayList<String>(); //Agents that live/work in the zone.
		//Read the link ids from a txt file
		Scanner scanner = null;
		try {
			//originalNoCarZone
			//scanner = new Scanner(new File("/Users/haowu/Documents/Planung_und_Betrieb_im_Verkehrswesen/SS2020/Multi-agent_transport_simulation_SoSe_2020/HA/HA2/Run/resident_and_worker_Agents_HA2.txt"));


			//All Agents
			//scanner = new Scanner(new File("/Users/haowu/Documents/Planung_und_Betrieb_im_Verkehrswesen/SS2020/Multi-agent_transport_simulation_SoSe_2020/HA/HA2/experiment_smallNoCarZone/firstArea/SelectedAgent.txt"));

			//Only Residents
			//scanner = new Scanner(new File("/Users/haowu/Documents/Planung_und_Betrieb_im_Verkehrswesen/SS2020/Multi-agent_transport_simulation_SoSe_2020/HA/HA2/experiment_smallNoCarZone/firstArea/SelectedResidents.txt"));


			//No Car Zone from shape file
			scanner = new Scanner(new File("/Users/haowu/Workspace/fixedRepository/matsim-berlin-dz/scenarios/berlin-v5.5-1pct/input/TxtFile/resident_and_worker_Agents_HA2.txt"));
...
/*		//
		...
		for (Person person : scenario.getPopulation().getPersons().values()) {
			if(person.getId().equals(Id.createPersonId("200867101"))){
				System.out.println(person.getAttributes().getAttribute("subpopulation"));
			}
		}*/

		//System.out.println("prepareScenario works!");
		for (Person person: scenario.getPopulation().getPersons().values()) {
			//String subPop = (String) person.getAttributes().getAttribute("subpopulation");
			//System.out.println("the first for loop works");

/*			for(String id: agentIDList){
				//System.out.println("the second for loop works" + id);
				if(Id.createPersonId(id).equals(person.getId())){
					//System.out.println("person equals!!!");
					person.getAttributes().putAttribute("subpopulation", "personInternal");
				}
			}*/
			if (agentIDList.contains(person.getId().toString())){
				person.getAttributes().putAttribute("subpopulation", "personInternal");
			}
		}
