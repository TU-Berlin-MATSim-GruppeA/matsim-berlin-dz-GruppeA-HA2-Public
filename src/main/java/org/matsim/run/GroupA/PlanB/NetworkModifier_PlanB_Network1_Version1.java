 ...
 * @author dziemke
 */
public class NetworkModifier_PlanB_Network1_Version1 {
    private static final Logger LOG = Logger.getLogger(NetworkModifier_PlanB_Network1_Version1.class);

    public static void main (String[] args) throws IOException {
        // Input and output files
        String networkInputFile = "scenarios/berlin-v5.5-1pct/input/berlin-v5.5-network.xml.gz";
        String networkOutputFile = "scenarios/berlin-v5.5-1pct/input/PlanB/network-modified-carInternal_Network1.xml.gz";
        ...
        //******changes in PlanB_Network1_Version1******
        //——————Input——————
        ...
        //notCarModeList.addAll(InternalStreetsList); //only for second Network of PlanB
...
/*        // Store relevant area of city as geometry
        ...
        for (Link link : carInternalScenario.getNetwork().getLinks().values()) {
            ...
            if(carInternalList.contains(link.getId().toString())) {
                ...
                for (String mode : link.getAllowedModes()) { //Do we need this line???
                    //******changes in PlanB_Network1_Version1******//
            ...
            }else{ //for the links not in our focused zone
                for (String mode : link.getAllowedModes()) { //Do we need this line???
                    //******changes in PlanB_Network1_Version1******//
                    //allowedModesAfter.add("carInternal"); // This is the carInternal list, so carInternal needs to be re-added
                    //******changes in PlanB_Network1_Version1******
